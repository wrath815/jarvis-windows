/**
 * JARVIS-Windows — Multi-mode particle visualization.
 * Ported from original JARVIS orb.ts
 *
...
export function createOrb(canvas: HTMLCanvasElement): Orb {
  ...
  function animate() {
    ...
    else if (state === "compacting") zTarget = -20 + Math.sin(t * 0.08) * 3;
    ...
    if (state === "thinking") { 
      ...
      lineMat.color.lerp(speakingColor, 0.015); 
    }
    else if (state === "compacting") { 
      mat.color.lerp(compactingColor, 0.03); 
      lineMat.color.lerp(compactingColor, 0.03); 
    }
    else { 
